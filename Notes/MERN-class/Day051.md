# MERN CLASS NOTES — DAY 051

## 📆 Date: **February 5, 2026 (Thursday)**

---

## 🎯 Topic: CRUD Feature for Earthquake API (Thunder Client + MongoDB)
...
- Method: **PUT**
- URL: `http://localhost:8080/api/earthquakes/<mongodb_id_here>`
- Body → JSON (only the fields you want to change)
...
## 🧪 Testing Notes (Thunder Client)

### ✔️ Best practice testing order:
1. **POST** → create a new earthquake
2. Copy returned `_id`
3. **GET** all → confirm it exists
4. **GET by id** → confirm one document fetch
5. **PUT** → update fields
6. **DELETE** → remove it
...
### ❌ POST works but data not visible in Compass
Possible causes:
